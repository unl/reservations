...
get '/engineering_garage/user_agreement/?' do
  ...
  erb :'/engineering_garage/user_agreement', :layout => :fixed, :locals => {}
end
...
post '/engineering_garage/user_agreement/?' do
  ...
  # Process form data here; for instance, save the data to a database or perform other actions
  @user.set_expiration_date(today)
end

# Helper method to check if required params are present
helpers do
  def params_valid?
    required_params = %w[weapon-agreement cleaning-agreement priviledge-agreement question-agreement
                         active-cleaning-agreement ppe-agreement certification-agreement report-agreement]
