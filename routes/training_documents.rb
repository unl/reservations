...
get '/training-documents/sop/?' do
	...
	erb :sop_training_document, :layout => :fixed, :locals => {
		:content => Net::HTTP.get_response(URI.parse("https://innovationstudio.unl.edu/node/#{drupal_link_lookup(:sop_training_doc)}?format=partial")).body
	}
end
...
get '/training-documents/workshops/?' do
	...
	erb :workshops_training_document, :layout => :fixed, :locals => {
		:content => Net::HTTP.get_response(URI.parse("https://innovationstudio.unl.edu/node/#{drupal_link_lookup(:workshops_training_doc)}?format=partial")).body
	}
end
...
get '/training-documents/tips-and-tricks/?' do
	...
	erb :tips_and_tricks_training_document, :layout => :fixed, :locals => {
		:content => Net::HTTP.get_response(URI.parse("https://innovationstudio.unl.edu/node/#{drupal_link_lookup(:tips_training_doc)}?format=partial")).body
	}
end